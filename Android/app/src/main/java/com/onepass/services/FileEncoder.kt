...
class FileEncoder {
    fun load(password: String, input: InputStream): OnePassData {
        ...
        val keySpec = PBEKeySpec(password.toCharArray(), header.salt, PBKDF2_ITERATIONS, AES_KEY_BITS)
        val key = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA1").generateSecret(keySpec).encoded
    ...
    fun verify(password: String, input: InputStream): Boolean {
        ...
        return MessageDigest.isEqual(passwordHash(password, header.salt), header.passwordHash)
    }
    ...
    private fun passwordHash(password: String, salt: ByteArray): ByteArray =
        MessageDigest.getInstance("SHA-512").digest(password.toByteArray(StandardCharsets.UTF_8) + salt)
